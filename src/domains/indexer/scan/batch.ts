...
import { saveArtworkToCache } from "../metadata/artwork-cache"
import { normalizeMetadata } from "../metadata/normalize"
import { runWithScopeCommit, COMMIT_SCOPE_SIZE } from "./scope-commit"
import { upsertPreparedAsset, type IndexingLookupCache } from "./upsert"
  ...
  for (let attempt = 1; attempt <= METADATA_EXTRACTION_MAX_ATTEMPTS; attempt += 1) {
    ...
    try {
      metadata = await extractMetadata(
        ...
        asset.filename || "",
        asset.duration,
        splitConfig || getPreferenceState().splitMultipleValueConfig
      )
      break
    } catch (error) {
      lastError = error
