...
import { normalizeText } from "@/modules/indexer/metadata/normalization"
import { yieldToEventLoop } from "@/modules/indexer/utils/batch"
      ...
      for (const track of scope) {
        ...
        if (relationArtistIds.length > 0) {
          await db.insert(trackArtists).values(
            relationArtistIds.map((artistId) => ({ trackId: track.id, artistId }))
          )
        }
