...
- **Hub**: `/settings` stack (headerShown, cancel-to-close on index, back buttons on details) with in-hub search over `SETTINGS_SEARCH_ENTRIES` (routes + highlight hints); gear entry point added to the library tab header.
- **Screens**: appearance (theme/theme-mode/language), audio (transitions/resume/audio-focus/crossfade slider), library (folder filters w/ apply+rescan, track duration filter w/ custom slider, count-as-played slider, auto-scan toggles, reindex dialog), library-tabs reorderable list, split-multiple-values (tag editor sheets + artist split mode), notifications, advanced (log level, share logs via ported `core/log/file-appender`, history/search-history reset, force update mixes, battery optimization, dontkillmyapp, restart onboarding), backup/restore + auto-backup intervals, theme picker with mock-app previews, about + open-source licenses sheet.
- **Persistence**: all screens write through `preferenceStore.setState` directly; no per-module setter files. Language changes call `i18n.changeLanguage`.
