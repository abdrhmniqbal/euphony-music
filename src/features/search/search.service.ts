import { getAlbumsByGenre, getAllGenres, getAllSongsByGenre, getTopSongsByGenre, type AlbumInfo } from '@/db/operations';
...
export async function fetchGenreDetails(genreName: string): Promise<{ topSongs: Track[]; albums: AlbumInfo[] }> {
  const [topSongs, albums] = await Promise.all([
    getTopSongsByGenre(genreName, 25),
...
export async function fetchGenreTopSongs(genreName: string): Promise<Track[]> {
  return getAllSongsByGenre(genreName);
}
