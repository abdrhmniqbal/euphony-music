...
export async function restorePreferencesFromFile(uri: string): Promise<boolean> {
  try {
    ...
    const { setIndexerNotificationsEnabled } = await import("@/modules/settings/indexer-notifications")
    const { setTrackDurationFilterConfig } = await import("@/modules/settings/track-duration-filter")
    ...
    if (data.settings.folderFilterConfig) await commitFolderFilterConfig(data.settings.folderFilterConfig)
    if (data.settings.splitMultipleValueConfig) await setSplitMultipleValueConfig(data.settings.splitMultipleValueConfig)
    if (data.settings.appUpdateConfig) await setAppUpdateConfig(data.settings.appUpdateConfig)
    if (data.settings.audioPlaybackConfig) await setAudioPlaybackConfig(data.settings.audioPlaybackConfig)
    if (data.settings.indexerScanConfig) await setAutoScanConfig(data.settings.indexerScanConfig)
    if (data.settings.countAsPlayedConfig) await setCountAsPlayedConfig(data.settings.countAsPlayedConfig)
    if (data.settings.crossfadeConfig) await setCrossfadeConfig(data.settings.crossfadeConfig)
    if (data.settings.indexerNotificationsEnabled !== undefined) await setIndexerNotificationsEnabled(data.settings.indexerNotificationsEnabled)
    if (data.settings.trackDurationFilterConfig) await setTrackDurationFilterConfig(data.settings.trackDurationFilterConfig)
