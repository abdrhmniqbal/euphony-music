...
**Package manager:** Always use `bun` — the fast all-in-one JavaScript toolkit (runtime, package manager, bundler). Do not call `yarn`/`npm`/`pnpm`/`nub` directly for installs; use `bun install` and `bun run <script>`.

**Bun lockfile:** `bun.lock` is bun's managed lockfile (commit it). `bunfig.toml` holds bun config if needed.
...
**Checks:** `bun run lint` (oxlint), `bun run format:check` (oxfmt), `bun run test` (vitest), and `bun run check` (lint + test).
