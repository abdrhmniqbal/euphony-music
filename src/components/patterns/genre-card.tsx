...
const PATTERN_ELEMENTS: PatternElements = {
  ...
  grid: (
    <View className="absolute inset-0 flex-row flex-wrap gap-2 p-1.5">
      {Array.from({ length: 12 }, (_, i) => (
        <View key={i} className="h-6 w-6 rounded-sm bg-white/5" />
      ))}
    </View>
  ),
  diamonds: (
  ...
  crosses: (
    <>
      ...
      <View className="absolute bottom-7 left-2 h-8 w-2 rounded-full bg-white/10" />
    </>
  ),
}
    ...
    <PressableFeedback onPress={onPress} onLongPress={onLongPress} className="w-[47.5%] active:opacity-80">
      <Card className="relative h-24 justify-between overflow-hidden border-none p-4" style={{ backgroundColor: resolvedColor }}>
        ...
        <View pointerEvents="none" className="absolute inset-0">
          {PATTERN_ELEMENTS[pattern] ?? PATTERN_ELEMENTS.circles}
        </View>
      </Card>
    </PressableFeedback>
  )
