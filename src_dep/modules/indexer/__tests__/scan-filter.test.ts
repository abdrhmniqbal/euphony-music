...
import { normalizeMetadata, normalizeText } from "@/modules/indexer/metadata/normalization"

function asset(filename: string, uri = `file:///storage/emulated/0/Music/${filename}`): MediaLibrary.Asset {
...
describe("isSupportedAssetByExtension", () => {
  ...
  it("falls back to empty filename safely", () => {
    expect(isSupportedAssetByExtension({ filename: undefined } as unknown as MediaLibrary.Asset)).toBe(false)
  })
})
