...
function selectDominantArtwork(trackArtworks: string[]) {
  ...
  for (const [artwork, value] of stats.entries()) {
    if (value.count > selectedCount || (value.count === selectedCount && value.latestIndex > selectedLatestIndex)) {
...
export function buildArtistAlbums(artistTracks: PlayerTrack[]): ArtistAlbum[] {
  const albumMap = new Map<string, ArtistAlbum & { albumArtworks: string[]; trackArtworks: string[] }>()
